#!/bin/bash

# Скрипт для деплою виправлень платних заявок

echo "🚀 Починаємо деплой виправлень..."

# Перехід в директорію проекту
cd /var/www/sviydlyasvoih/platform-svoi

# Зупинка PM2
echo "⏸️  Зупиняємо PM2..."
pm2 stop sviy-platform

# Оновлення коду
echo "📥 Оновлюємо код..."
git stash
git pull origin main

# Встановлення залежностей (якщо потрібно)
echo "📦 Перевіряємо залежності..."
npm install

# Видалення старого білду
echo "🗑️  Видаляємо старий білд..."
rm -rf .next

# Генерація Prisma Client
echo "🔧 Генеруємо Prisma Client..."
npx prisma generate

# Білд проекту
echo "🏗️  Збираємо проект..."
npm run build

# Запуск PM2
echo "▶️  Запускаємо PM2..."
pm2 start ecosystem.config.js
pm2 save

# Очікування запуску
echo "⏳ Очікування запуску..."
sleep 3

# Перевірка логів
echo "📋 Перевірка логів..."
pm2 logs sviy-platform --lines 30 --nostream

echo ""
echo "✅ Деплой завершено!"
echo ""
echo "Перевірте що все працює: https://sviydlyasvoih.pp.ua"
