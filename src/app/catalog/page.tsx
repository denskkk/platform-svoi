                ...
                <Link key={u.id} href={`/profile/${u.id}`} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition group">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <p className="font-semibold text-gray-900 text-lg flex items-center gap-1.5 flex-wrap">
                        <span>{displayName}</span>
                        {u.isVerified && <span className="text-blue-600" title="Верифікований">✓</span>}
                        {(u as any).ucmVerified && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-100 text-green-700 text-xs font-bold rounded-full" title="Перевірено УЦМ">
